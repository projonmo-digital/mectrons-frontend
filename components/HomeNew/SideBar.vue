...
const onMouseOver = (item: any) => {
  console.log("Mouse over on:", item.label);
};
</script>
...
<template>
  <div class="w-[262px] bg-[#EAE5E2]/50 p-4 flex flex-col gap-8">
    ...
    <DropdownMenu class="bg-white" v-for="i in items" :key="i.label">
      <DropdownMenuTrigger as-child>
        <Button class="flex justify-between bg-white text-black">
          <div>
            {{ i.label }}
          </div>
      ...
      <DropdownMenuContent class="w-56 bg-white">
        <DropdownMenuItem v-for="j in i.subMenu" :key="j.label">
          {{ j.label }}
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
