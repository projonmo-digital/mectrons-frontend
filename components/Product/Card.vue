...
const bookmarkAdd = async (product) => {
  ...
  try {
    const { pending, data } = await useFetch(
      `${useRuntimeConfig().public.baseUrl}/bookmark/${id}`,
      {
        method: "PUT",
        headers: {
  ...
  <div class="card">
    ...
    >
      ...
      >
        <img
          ...
          :src="
            ...
            '/' +
            props.product?.picture[0].replaceAll('public', 'storage')
          "
          alt="Product"
        />
        <img
      ...
      <div v-if="auth?.user?.id != props.product?.user_id">
        <span
          v-if="props.product.is_bookmarked == 0"
          @click="bookmarkAdd(props.product)"
        ...
        <span
          v-else
          @click="bookmarkRemove(props.product)"
          class="absolute top-2 left-3 rounded-full text-center text-2xl font-medium text-dark-900"
        >
          <i class="fa-solid fa-heart"></i>
        </span>
      </div>
      <div class="mt-4 px-5 pb-5">
        ...
        <div class="flex items-center mt-3">
          <nuxt-link :to="`/product-details/${props.product?.id}`">
            <h5 class="text-sl font-bold tracking-tight card-title">
              {{ props.product?.title }}
            </h5>
          </nuxt-link>
        </div>
        ...
        <div class="mt-3 mb-5 flex items-center justify-between">
          <div class="flex items-center gap-x-1">
            <span
              class="text-sm px-1 py-0.5 bg-[rgba(245,_127,_32,_1)] text-white rounded-lg"
              >-30%</span
            >
