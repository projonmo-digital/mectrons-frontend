    ...
    <div class="card">
        <div class="relative flex w-full max-w-xs flex-col overflow-hidden rounded-lg border border-gray-100 bg-white shadow-md">
            ...
            <div v-if="auth?.user?.id != props.product?.user_id">
                <span v-if="props.product.is_bookmarked == 0" @click="bookmarkAdd(props.product,index)" class="absolute top-2 left-3 rounded-full text-center text-2xl font-medium">
                    <i class="fa-solid fa-heart"></i>
                </span>
                <span v-else @click="bookmarkRemove(props.product,index)" class="absolute top-2 left-3 rounded-full text-center text-2xl font-medium text-[rgb(223,_120,_37,_1)]">
                    <i class="fa-solid fa-heart"></i>
                </span>
            </div>
