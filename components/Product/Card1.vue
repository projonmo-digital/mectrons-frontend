    ...
    <div class="card bg-gray-900/20 rounded-lg">
        <div class="flex gap-4">
            <div class="relative w-28 h-36 md:h-36 md:w-44">
                <nuxt-link :to="`/product-details/${props.product?.id}`" class="flex justify-center items-center overflow-hidden">
                    <img  class="object-cover rounded-t-lg w-28 h-36 md:h-36 md:w-44 md:rounded-none md:rounded-s-lg" v-if="props.product?.picture != ''" :src="useRuntimeConfig().public.imageUrl+'/'+props.product?.picture[0].replaceAll('public','storage')" alt="Product" />
                    <img  class="object-cover rounded-t-lg w-28 h-36 md:h-36 md:w-44 md:rounded-none md:rounded-s-lg" v-else src="assets/images/dummy-image.jpg" alt="Ads" />
                </nuxt-link>
            ...
            <div class="flex flex-col justify-between py-2 pe-2 w-[calc(100%-11rem)] md:w-[calc(100%-13rem)]">
                ...
                <div class="flex items-center gap-x-1">
                    ...
                    <div class="flex items-center gap-x-2">
                        <span class="text-lg font-bold  text-[rgba(215,_14,_14,_1)]">{{ props.product?.currency?.symbol }}{{ product?.price }}</span>
                        <span class="text-md line-through text-gray-400">$180.00</span>
                    </div>
                </div>

                <div class="flex mt-3">
                    <button @click="AddToCart(props.product)" type=" submit" class="text-sm focus:outline-none text-white bg-[rgba(245,_127,_32,_1)] hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-full text-sm px-3 py-1.5  dark:focus:ring-yellow-900">
                        Add to cart
                    </button>
                </div>
            </div>
