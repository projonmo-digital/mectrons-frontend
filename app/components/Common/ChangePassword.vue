...
const handleSubmit = async () => {
    try {
        ...
        const response = await $fetch<any>(
            `${useRuntimeConfig().public.baseUrl}/change-password`,
            {
                method: "post",
                headers: {
...
<template>
    <form class="space-y-6" @submit.prevent="handleSubmit">
        <div>
            ...
            <div class="relative">
                <input tabindex="0" :type="passHideShow ? 'text' : 'password'" class="w-full border p-2" name="password"
        ...
        <div>
            ...
            <div class="relative">
                <input tabindex="0" type="password" class="w-full border p-2" name="password" id="password"
                    placeholder="password" v-model="form.password_confirmation" />
            </div>
            <span v-if="Object.keys(auth.errors).includes('password_confirmation')" class="text-sm text-red-500">{{
                auth.errors.password_confirmation[0] }}</span>
        </div>
