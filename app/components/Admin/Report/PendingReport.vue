...
import ProductInfo from '~/components/Admin/Report/ProductInfo.vue'
...
const fetchData = async (params = {}) => {
    ...
    try {
        ...
        let url = `${useRuntimeConfig().public.baseUrl}/report/pending?${new URLSearchParams(params).toString()}`;
...
const columns: ColumnDef<any>[] = [
  {
    ...
    cell: ({ row }) => {
      return h(ProductInfo, {
        class: "",
        modelValue: row.original.product
      });
    }
  },
  {
    accessorKey: "subject",
    header: () => h("div", { class: "text-start" }, "Subject"),
  },
  {
    accessorKey: "desc",
    header: () => h("div", { class: "text-stert" }, "Description"),
  },
  {
    ...
    cell: ({ row }) => {
      return h(UserListViewItem, {
        class: "",
        modelValue: row.original.reporter
      });
    }
  },
  {
